...
class RefundService:
    ...
    async def create_refund_request(
        ...
        # Refund evidence must belong to the same authenticated customer.
        # Evidence URLs are generated by the upload endpoint and stored as
        # relative /uploads paths; reject arbitrary external URLs.
        if actor_role == "customer":
            for evidence_url in data.evidence or []:
                if not str(evidence_url).startswith("/uploads/"):
                    raise RefundNotEligibleError("Invalid refund evidence reference")
