...
class AgriConnectLLMPlanner:
    ...
    async def plan(self, message: str, conversation: Optional[List[Dict[str, Any]]] = None):
        ...
        # Keep the schema as a normal Python string. Escaping the JSON quotes
        # twice (e.g. \\"requests\\") makes the source invalid Python and
        # prevents the entire AI package from importing.
        schema = (
            '{"requests":[{"intent":string,"query":string,"product":string,'
            '"days":7,"destination":string,"open":false}]}'
        )
        prompt = (
            "You are the AgriConnect website assistant. Convert the user request to JSON only. "
            "Intents: product_search, cheapest_product, demand_forecast, price_prediction, "
            "marketplace_statistics, delivery_information, traceability, navigate, "
            "project_information, general. "
